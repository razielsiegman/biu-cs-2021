...
public class NuSMVInterface{
    ...
    public NuSMVInterface(String path,Converter c)throws Exception{
        ...
        ProcessBuilder b = new ProcessBuilder(new String[]{"nuXmv","-int",path});
...
//    }
    ...
    private String checkLTL_BMC_0(String spec)throws Exception{
        ...
        spec = spec.replaceAll("\\n", "").replace("@"," ");
        //execute command
        out.println("msat_check_ltlspec_bmc -k 0 -l X  -p \""+spec+"\"");
        out.flush();
        String s = readPrompt();
        return s;
    }
...
//    }
    ...
    private String checkLTL_BDD(String spec)throws Exception{
        ...
        spec = spec.replaceAll("\\n", "").replace("@"," ");
        //execute command
        out.println("check_ltlspec_inc_coi_bdd -p \""+spec+"\"");
        out.flush();
        String s = readPrompt();
        return s;
    }
    ...
    private String readPrompt(){
        ...
        while((l=in.next())!=null){
            if (l.startsWith("nuXmv"))  {in.next();break;}//the call to next() reads past the ">" character
